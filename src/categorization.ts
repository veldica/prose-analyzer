...
export function isSensory(word: string, isNormalized = false): boolean {
  if (!word) return false;
  if (isNormalized) {
    return SENSORY_WORDS.has(word);
  }
  return SENSORY_WORDS.has(word.toLowerCase().replace(/[^a-z]/g, ""));
}

export function isAbstract(word: string, isNormalized = false): boolean {
  if (!word) return false;
  if (isNormalized) {
    return ABSTRACT_WORDS.has(word);
  }
