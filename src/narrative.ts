 ...
 * Uses a non-greedy approach to avoid backtracking bombs.
 */
...
export function analyzeNarrative(
  ...
  const cleanWords = wordsRaw.map(w => w.toLowerCase().replace(/[^a-z]/g, "")).filter(w => !!w);

  const sensoryCount = cleanWords.filter((w) => isSensory(w, true)).length;
  const sensoryDensity = round(safeDivide(sensoryCount, wordCount), 4);

  const abstractCount = cleanWords.filter((w) => isAbstract(w, true)).length;
