...
- **Length-Independent Metrics**: Implements MATTR (Moving Average TTR) to accurately measure vocabulary richness in texts of any length, from single paragraphs to full chapters.

## Features

- **High-Precision Lexical Analysis**: Robust diversity metrics like MATTR that account for text length bias.
- **Narrative Texture Heuristics**: Deterministic proxies for "active" vs. "passive" writing styles.
- **Linguistic Signal Detection**: Categorization of sensory and abstract language usage.
- **Zero Opaque Logic**: No LLMs or stochastic models; 100% deterministic and explainable results.
...
Checks if a word belongs to the sensory language category.

### `isAbstract(word: string): boolean`
Checks if a word belongs to the abstract concepts category.
